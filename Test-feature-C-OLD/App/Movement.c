...
int Movement_Init(struct Coordinate_Class Origin_Coor, struct Interpolation_Parameter_t Input_Para, struct Interpolation_Parameter_t Interpolation_Parameter)
{
	...
	Origin_Coor_InWorld.angle_coor  =  fabs(Origin_Coor_InWorld.angle_coor); //保证角度的值为正
	...
	//printf("Destination_Coor_InOrigin.angle_coor = %f , Origin_Coor_InWorld.angle_coor = %f\n" ,Destination_Coor_InOrigin.angle_coor , Origin_Coor_InWorld.angle_coor);
    //printf(" temp_result = %f \n", temp_result);
	...
	if (abs(Destination_Coor_InOrigin.x_coor) > 50 || abs(Destination_Coor_InOrigin.y_coor) > 50 || abs(Destination_Coor_InOrigin.angle_coor) > 50)
    {
       //  float input_distance_abs = Cal_Destination_Displacement(Destination_Coor_InOrigin);   //计算待插补的距离
		float input_distance_abs = fabs(Destination_Coor_InWorld.x_coor - Virtual_AGV_Current_Coor_InWorld.x_coor ) * (Destination_Coor_InWorld.x_coor - Virtual_AGV_Current_Coor_InWorld.x_coor ) + (Destination_Coor_InWorld.y_coor - Virtual_AGV_Current_Coor_InWorld.y_coor ) * (Destination_Coor_InWorld.y_coor - Virtual_AGV_Current_Coor_InWorld.y_coor )   ;
		input_distance_abs = sqrtf (input_distance_abs) ;
...
float Cal_Destination_Displacement(Coordinate_Class_t Destination_Coor_InOrigin)
{
    ...
    printf("angle_equivalent_temp_InOrigin =  %f\n  ", angle_equivalent_temp_InOrigin );
...
int Movement_Cal_Velocity(Coordinate_Class_t Current_Coor_InWorld, struct Interpolation_Parameter_t Interpolation_Parameter)
{
	...
	float current_distance =  sqrtf( (AGV_Current_Coor_InWorld.x_coor - Virtual_AGV_Current_Coor_InWorld.x_coor) * (AGV_Current_Coor_InWorld.x_coor - Virtual_AGV_Current_Coor_InWorld.x_coor) + (AGV_Current_Coor_InWorld.y_coor - Virtual_AGV_Current_Coor_InWorld.y_coor) * (AGV_Current_Coor_InWorld.y_coor - Virtual_AGV_Current_Coor_InWorld.y_coor));
	//current_distance = Cal_Current_Coor_InOrigin(Target_Coor_InOrigin);     //获取目标坐标在起点坐标下的距离
	printf(" Current_Coor_InOrigin.x_coor = %f , Virtual_AGV_Current_Coor_InWorld.x_coor = %f\n ", AGV_Current_Coor_InWorld.x_coor  , Virtual_AGV_Current_Coor_InWorld.x_coor);
