...
void Movement_Control()
{
    ...
    if (Add_Command_Line == 1)
    {
        if ((Error_Coor_InAGV.x_coor < 7.0) && (Error_Coor_InAGV.y_coor < 3.0) && (Error_Coor_InAGV.angle_coor < 10.0) && (Error_Coor_InAGV.x_coor > -7.0) && (Error_Coor_InAGV.y_coor > -3.0))
        {
            VL_1200 = 0.0;
            VR_1200 = 0.0;
            stop = 1;
        ...
        else
        {
        	...
        	if (abs(vx - PID_result) - abs(vx + PID_result) > 2)
        		{
        			printf("偏里\n");
        			VL_1200 = vx + PID_result;
        			VR_1200 = vx - PID_result+20*Distance_Symbols;
        			stop = 0;
        		}
        	else
        	     {
